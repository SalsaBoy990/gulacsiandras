(function () {
  ...
  // Build topic subpages
  // cwd: current working directory
  const topics = glob.sync('**/*.ejs', {
    cwd: `${srcPath}/pages/topics/`
  })
  ...
  try {
    topics.forEach((file) => {
      ...
      // make directory
      fse.mkdirsSync(`${destPath}/temakor/`)

      // read data from file and then render page
      const pageContents = ejsRender(fse.readFileSync(`${srcPath}/pages/topics/${file}`, 'utf-8'), Object.assign({}, config, postsDataForIndexPage, {
        postsDataForIndexPage: postsDataForIndexPage
      }))
      ...
      // read layout data from file and then render layout with page contents
      switch (name) {
        case 'eghajlatvaltozas.ejs':
          layoutContent = ejsRender(fse.readFileSync(`${srcPath}/layouts/default.ejs`, 'utf-8'), Object.assign({}, config, {
            title: 'Éghajlatváltozás | ' + config.site.title,
            body: pageContents,
            canonicalUrl: config.site.url + '/' + fileData.name,
            description: config.site.quote
          }), { filename: `${srcPath}/layouts/default.ejs` })
        ...
        case 'energetika.ejs':
          layoutContent = ejsRender(fse.readFileSync(`${srcPath}/layouts/default.ejs`, 'utf-8'), Object.assign({}, config, {
            title: 'Energetika | ' + config.site.title,
            body: pageContents,
            canonicalUrl: config.site.url + '/' + fileData.name,
            description: config.site.quote
          }), { filename: `${srcPath}/layouts/default.ejs` })
        ...
        case 'eroforrasok.ejs':
          layoutContent = ejsRender(fse.readFileSync(`${srcPath}/layouts/default.ejs`, 'utf-8'), Object.assign({}, config, {
            title: 'Erőforrások | ' + config.site.title,
            body: pageContents,
            canonicalUrl: config.site.url + '/' + fileData.name,
            description: config.site.quote
          }), { filename: `${srcPath}/layouts/default.ejs` })
        ...
        case 'mezogazdasag.ejs':
          layoutContent = ejsRender(fse.readFileSync(`${srcPath}/layouts/default.ejs`, 'utf-8'), Object.assign({}, config, {
            title: 'Mezőgazdaság  | ' + config.site.title,
            body: pageContents,
            canonicalUrl: config.site.url + '/' + fileData.name,
            description: config.site.quote
          }), { filename: `${srcPath}/layouts/default.ejs` })
        ...
        case 'pszichologia.ejs':
          layoutContent = ejsRender(fse.readFileSync(`${srcPath}/layouts/default.ejs`, 'utf-8'), Object.assign({}, config, {
            title: 'Pszichológia | ' + config.site.title,
            body: pageContents,
            canonicalUrl: config.site.url + '/' + fileData.name,
            description: config.site.quote
          }), { filename: `${srcPath}/layouts/default.ejs` })
        ...
        case 'tulnepesedes.ejs':
          layoutContent = ejsRender(fse.readFileSync(`${srcPath}/layouts/default.ejs`, 'utf-8'), Object.assign({}, config, {
            title: 'Túlnépesedés | ' + config.site.title,
            body: pageContents,
            canonicalUrl: config.site.url + '/' + fileData.name,
            description: config.site.quote
          }), { filename: `${srcPath}/layouts/default.ejs` })
        ...
        default:
          throw new Error('Something went wrong with topic subpages:')
      }
      // save the html file
      fse.writeFileSync(`${destPath}/temakor/${fileData.name}.html`, layoutContent)
    })
    console.log('Topic subpages OK...')
  } catch (err) {
    console.error(err)
    console.error('Build topic subpages failed...')
  }
